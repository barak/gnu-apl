...
{
   providers.insert(pair<const string, Provider *>
   ...
   loop(i, num_args)
       {
        ...
        else if (cell_B.is_float_cell())
            {
              arg_list->append_double(cell_B.get_real_value(), i);
            }
        else
           {
             Value_P value = cell_B.to_value(LOC);
             if (value->get_shape().get_volume() == 0)
                ...
                {
                  const UTF8_string utf(value->get_UCS_ravel());
                  arg_list->append_string(utf.c_str(), i);
                }
            else
                {
...
static Value_P run_generic( Connection *conn, Value_P A, Value_P B, bool query )
{
    if( !A->is_char_string() ) {
     ...
     const UTF8_string A_utf(A->get_UCS_ravel());
     string statement = conn->replace_bind_args(A_utf.c_str());
    ...
    if (shape.get_rank() == 0 || shape.get_rank() == 1)
       ...
       {
         const int rows = shape.get_rows();
         const int cols = shape.get_cols();
         if (rows == 0)
            {
              return Idx0(LOC);
            }
              ...
              for (int row = 0 ; row < rows ; row++)
                  {
                    ...
                    result = run_generic_one_query(arg_list.get(), B, row*cols,
                                                   cols, not_last);
                    if (not_last)   arg_list->clear_args();
                  }
               return result;
            }
       }
       ...
       {
         MORE_ERROR() << "Bind params have illegal rank > 2";
         RANK_ERROR;
       }
}
//-----------------------------------------------------------------------------
