     ...
     errors(0)
{
   ...
   if (ws_name.compare(UCS_ASCII_string("CLEAR WS")) == COMP_EQ)
   ...
   else if (Avec::is_digit(ws_name[0]))
      {
         ...
         ws_name = UCS_string(ws_name, pos);
      }

   root_dir << "/" << UTF8_string(ws_name);
...
UTF8_string fun_filename(root_dir);
   fun_filename << "/f_" << UTF8_string(alias) << ".html";
...
UTF8_string cg_filename(root_dir);
   if (caller)   cg_filename << "/gc_";
   else          cg_filename << "/cg_";
   cg_filename << UTF8_string(alias);
UTF8_string png_filename(cg_filename);
UTF8_string cmapx_filename(cg_filename);
   cg_filename << ".gv";
   png_filename << ".png";
   cmapx_filename << ".cmapx";
