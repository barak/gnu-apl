...
and move mouse to window witin 5 seconds
...
NOTE: newer glade versions behave slightly different, but the description
given here should suffice to explain glade. For *glade alternatives*, see also
<<TAG_programmatically, Chapter 8 Specifying GUIs Programmatically>> below.
...
the .css (if any) files are reasonablt small. It makes therefore a lot of
...
as a starting point for your own, self-contained GUI. Thios workspqace is
resonably well documented, but some related concepts may be helpful note
...
exposes the primary functionality of *glade*: producing XML temlates and filling
...
As already meantioned, *glade* produces an XML file that describes the desired
...
of the widgets. Every relevant naming convertion (XML use only some of
...
characters are part of a core name. These characters are instead used to derivw
...
Every widget is an instance of an assoctiated class which is declared in the
...
then not be mentioned in the XML. The relevant execeptions (in which cases
...
The object ID of a widge consists of an *ID-prefix* and an *ID-suffix*
...
Then the second ⎕GTK call hopefully does this:
...
4. In step 3. the function arguments and the function result (if any) are being
   transcoded between APL values and C values. For this reason, only widget
...
Gtk_server) is that the new window title *MY-TITLE* does not show immeidately
...
is achieved by XML tag <property>/ In the case of the windiw title the
...
the proper *MY-TITLE* was set *before* the *window* was created. Properies
...
for certain data types like numbers or the booleans *True* and *False*.
...
* the properties *width_request* and *height_request* specify the desired
  sizes of the parent widget *window1* and its (so far) anonynmous child
  of type *GtkButton*.
* the properties *halign* and *valign* of the button with value *start* tell
  ...
  horizontally resp. top vercically) of the parent. Without them the button
...
button at a different offset, say *x* pixels rigght and *y* pixels down from
...
statments. The key to success with CSS is to keep matters simple, and
...
the dtylr (i.e. colors) above. For the top-level window that is normally
...
class *inluding their sub-widgests* while a widget name as  CSS selector
selects the widget(s) with that name *exluding their sub-widgests*.
...
but has no connectin to APL yet. Let's say we want to close the window when
the OK button is being pushed. This almost inevitably requires some sort of
waiting loop (where APL waits for the user to press the OK button).

Before APL can receive events such as user clicks on a button, these event.
must be forwarded to APL. Without that, events remain inside GTK (so the
button looks alive and, with the CSS above, changes its font when the mouse
moves over it) but APL will not be notified. We therefore have to connect the
event we are interested in to a handler that forwards it to APL. This is done
in the same way as in glade:
...
After that, there are a number of ways as to how to process the events
received from the GUI in APL. A rather simple but crude way is to way for
any event from GTK without checking it. A general problem with events is
that if you wait for a specific event, then before that event there could
be other events so that the waiting must be repeated. This repetition
almost inevitably requires a defined function (or at least a lambda) because
loops cannot be defined in immediate execution mode (and hence in APL
scripts). At least in standard APL (but see below for GNU APL).

----
      Event←⎕GTK 1   ⍝ blocking wait for any (!) event from GTK
      H ⎕GTK 0       ⍝ close the window
----
...
* The handle for the Gtk window (integer)
* the name of the widget sending the event (APL string), and
...
though, it may be worthwile to have a look at the workspace
...
and will be maintained in a backward-compatible fashion. In contrst, the
...
displays the mapping between widget functioin numbers and function names (of
that Gtk_server).
