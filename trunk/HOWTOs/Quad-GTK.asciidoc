...
1. In most cases the widget function is a function of the widget itself.
   ...
   for a widget usually lists the ancestor(s) of the widet under the topic
   ...
   *H_ID* for some button, then this would ask the Gtk_server to call the
   ...
   ancestor class that actualy defines the function, separated by ::.
...
no loop (which repeates if the wrong signal was received) is required
