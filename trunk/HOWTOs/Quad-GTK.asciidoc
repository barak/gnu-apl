...
simple examples, how *⎕GTK* based GUIs can be created and managed from GNU APL.
...
function would create an almost unusable API at the APL level. Even the
approach used for ⎕FIO or ⎕SQL - using function numbers as APL axis elements -
reaches its limits when used with almost 10000 functions.
...
more with the number of actions (like clicking, marking text, typing text, etc.)
...
This discrepancy between the huge number of functions provided by Gtk and
...
** Suitable *XML files* can be created with the GUI construction tool *glade*
   (which comes with GTK), while
...
//-
  This requirement reduces the need for all functions that are concerned with
  the instantiation of the GUI elements and their properties. It also speeds
  up the design of the GUI itself.
...
. The *Gtk_server* is started by *⎕GTK*, and after that *⎕GTK* communicates with
  ...
  in an "event" (= a TLV buffer) received by APL while a "command" (sent from
  ...
  are handled by the Gtk_server server process (one of the Gtk libraries) and
  are not communicated to APL. The encoding and decoding of TLVs that are
  exchanged between *apl* and *Gtk_server*) takes place inside *⎕GTK* so that
...
2. The programmer writes a CSS specification for the widgets in the GUI.
   The CSS file specifies the styling (i.e. colors. sizes etc.) of the
   widgets. In theory, this step is optional, but in practice it is inevitable
   for a somewhat good-looking GUI.

3. The programmer writes a brief start-up routine for the GUI in which the
   files created in steps 1. and 2. above are loaded into the GUI.

4. The programmer displays the GUI - either at the start of the APL
   application if the GUI shall be permanently visible or at some later time
   if the GUI is, for example, only popped up to request input from the user
   of the APL application. In the latter case the GUI is a single window
   (a "modal dialog") which is polled from APL until some final action of the
   user has occurred, while in the former case the APL application has to poll
   the GUI for events at rather short intervals. To poll *⎕GTK* at regular
   intervals:
...
* Every APL application that has a GUI (and uses *⎕GTK*) starts its own
  ...
  APL. Multiple Gtk_servers are independent of each other, and the coordination
  of events between the windows must be performed in the APL application.
...
* Normally larger amounts of data are not sent in events, but in command
  ...
  by means of a corresponding retrieval command. The new text is then sent from
...
** the retrieval commands are usually needed anyway. It is often easier
  ...
  the APL application and,
** the events become more uniform (not differing by different types of
  content). This, in turn, allows the same event to be used by different
  widgets that differ only by the type of data that they produce.
...
* Every signal is a TLV buffer whose length is dictated by the length of the
  ...
  integer tag and the value bytes V; the length is then ⍴V. In APL it is
  ...
  GNU APL system functions 33 ⎕CR and 34 ⎕CR have been tailored to encode and
  decode tag/value vectors TV to/from respectively the TLV buffers exchanged
...
* Receiving from Gtk_server is done with a single read() call (like ⎕FIO[43])
...
You will now see a new rectangular area in the *GUI visualization area*,
...
This grid is a helper-widget for controlling the placement of other widgets
inside *window1*.
...
Then *SAVE THE GUI !!!* in *glade*'s menu-bar under *File → Save As* and give
it the filename *my-application.ui*.
...
There exist numerous functions for controlling how widgets look (colors, fonts,
sizes, etc). However, constructing a GUI using these functions is a tedious
...
It specifies that buttons should, in general, have a white background and a
red foreground (= text color), but that the button with the name *"OK-button"*
...
fread() if you know what you are doing. This practice is, however, dangerous
because it might create inconsistencies between *⎕GTK* and ⎕FIO. For example,
closing a *⎕GTK* handle with ⎕FIO is technically possible, but usually a bad
idea.

Please note that a window can normally be closed from two ends: from APL with
...
place a new widget in *glade*, then *glade* assigns a unique ID to the widget
and makes sure that the id remains unique.
...
The ID consists of a class name followed by an instance number, for example
...
* the axis argument is: *H_ID ← H, ID* where H is the GUI handle and ID is
  the ID discussed above.
...
functions are implemented in *⎕GTK* and Gtk_server. However, the mapping from
...
crude (just good enough to demonstrate the access to a widget's content), but
we will see a smarter way in the next chapter (Managing Events). For the moment
we simply give the user 20 seconds time to enter data in the "Position:" entry
and read the entry data after that time has elapsed (regardless of whether the
...
* *B~int~* if the corresponding Gtk function was successful but returns no data
  (which is typical for all set_XXX() functions like set_text() above), or
...
* a vector *B~int~, DATA* if the corresponding Gtk function was successful and
  has returned the result *DATA* (which is typical for all *get_XXX()*
  functions such as *get_text()* above).

Therefore, even though some Gtk functions (i.e. those having a C/Cpp result
...
If an application program having a GUI is written in C/Cpp, then events such
as pressing a button or a key on the keyboard are usually forwarded to the
...
* *⎕GTK* is then used to poll events out of the event queue. The event queue in
...
* for every signal with arguments a1, a2, ... aN of type t1, t2, ... tN in Gtk
  there is at least one matching callback in Gtk_server to which the signal
  can be connected, and
* for every widget that can emit N different signals with the same argument
...
signals emitted by a widget can be connected to different callbacks. If a set
of callbacks satisfy the above then they:

* make Gtk_server more compact, and
* reduce the size of the APL function(s) decoding the events, but
* possibly decrease the readability of the event decoder because a signal
...
. the APL application polls the event out of the queue. At this point
  ...
  widgets as already described above.
...
* select the *button1* widget by clicking onto the OK button in the
  *GUI visualization area*. The single widget *window1* in the widget
  structure area (top-right of the *glade* window) will open and show its
  children. One of the children - *button1*  is now shown as being selected.
...
return an event to APL immediately if the event has occurred already (e.g.
the user has clicked *button1* _before_ the event queue was polled), and
otherwise wait until that event has occurred. *Non-blocking* means that the
...
The invocation for a blocking poll is *Z←⎕GTK 1*; The invocation for a
...
* *G* is an integer scalar for the handle of the GUI window in which the event
  was created,
...
Since our GUI only has one relevant event (clicking the OK button; the editing
of the Employee: and Position: fields is managed internally by Gtk), we keep
things simple and use a blocking poll of the *⎕GTK* event queue.
...
remove the H_ID variable (which was only used to make Step 5 above explicit),
and we hide useless return values from  *⎕GTK 1* and *⎕GTK[] "get_text"* with ⊣:
...
To simplify the drawing onto a canvas from APL, *⎕GTK* provides special
...
drawing the red circle. A real-world example would, of course, consist of
...
In both cases *DrawCmd* is a 4-element vector of strings. Once the
*DrawCmd* strings are constructed they are sent to the drawing area widget:
...
* Transparent colors are specified as a 4-element vector containing the red,
  green, and blue component of each color (0-255), and an opacity value
...
** The opaqueness of the line and fill colors can be used to control how a
  2-dimensional object is painted:
...
* Lines are 1-dimensional and therefore have no fill color. They are painted
  with the line color.
* To simplify the drawing of objects, colors are not assigned to
  individual objects, but are sent once using one of the following commands:
** *background R G B* 
** *fill-color R G B [A]*
** *fill color R G B [A]*
...
reasonably well documented, but some related concepts may be helpful to note
...
4. → select the new window and set the (topmost) property *ID*
     in tab *General* to *window1*
   
5. → *Save as* the project as *my-gui*.
...
exposes the primary functionality of *glade*: producing XML templates and
filling in fields in those templates according to choices made by the user (the
window ID *window1* in this simple example). The two *<child>* tags with content
*<placeholder/>* are used by *glade* to simplify a later replacement with
other widgets and have no effect.

File *my-gui.glade* is already a fully functional GUI (albeit empty and thus
boring) that can be used in APL via *⎕GTK*:
...
Now, let's test the claim above: _the filename (APL string) arguments *A*
...
* Every line becomes a nested (string) component of the multi-line string. The
...
* the XML standard treats newlines and blanks alike, therefore the string
  ...
  no corresponding ⎕CR conversion exists, so *36 ⎕CR* was used and long XML
  strings using \n are easier to read than those using blanks or TABs.


All properties of the new windows, like its size, caption, etc. are set to
their defaults. We will explain, by means of examples, how to change that in
...
You can use *⎕GTK* without any C/Cpp knowledge, but looking at some of the
...
In variable XML above, we have already seen the first (and so far only)
...
compound names: *gtktextview* and *gtktreeviewcolumn*. We call the
names *window*, *gtktextview*, and *gtktreeviewcolumn* the *core name*s
of the widgets. Every relevant naming convention (XML uses only some of
...
Depending on the platform, all these header files are most likely located in
...
Confusingly, many widgets not only declare the class name as described, but
...
* The *ID-suffix* is the trailing characters of that name, i.e. the suffix
...
In Cpp interfaces to GTK a widget like *window* is declared in header file
*window.h*, declare a *namespace Gtk*, a Cpp class *Window* inside namespace
*Gtk*, and member functions fun1, fun2, ... of that class.

⎕GTK uses the C interface instead and the Cpp luxury of namespaces and classes
become function name prefixes. This makes

----
    Gtk::Window::fun1(args...)                in Cpp, and
...
4. In step 3. the function arguments and the function result (if any) are
   trans-coded between APL values and C values. For this reason, only widget
   functions whose arguments and/or results are integer (including enums) or
...
1. In most cases the widget function is a function of the widget itself.
   ...
   *Hierarchy* and/or *Ancestors*. Note that the parent/child relation in
   ...
   would maybe like to call the widget function that tells us, which is
   ...
   *H_ID* for some button, then this would ask the Gtk_server to call the
   ...
   function exists. Instead we should have asked the Gtk_server to call the
...
that case the result Z is a (possibly mixed) vector. The first vector component
*↑Z* is the function number (APL integer) of the function that has returned
the result. The rest *1↓Z* is the result itself (an APL integer, APL Float,
or a (not enclosed) string.


=== 8.4 Adding Window Properties

The XML for a widget in a GUI defines how the widget looks initially.
...
property is called *title* (another GTK naming convention):
...
are somewhat widget-specific, but most widgets have at least the XML <property>
...
* the properties *halign* and *valign* of the button with value *start* tell
  the parent widget that the button shall be placed at the start (= horizontal
  left and vertical top resp.) of the parent. Without them the button
...
GTK handles the placement of child widgets of a parent widget by means of
...
should be placed into their common parent. We use the simplest of them:
*GtkFixed*.
...
The widget placement belongs to class *GtkFixed*, and happens in a separate
...
Nowadays the size of style files has grown to such an extent that it is no
longer possible to specify the style of a button or other HTML widget inline
...
The style attributes define how a widget looks (color, borders, etc.),
...
Let's start simple (with variable XML as above):
...
More complicated situations typically require looping in APL and are then not
...
and it may, since no APL loop is required, even be used in immediate execution
...
By default, GTK widgets send events (signals) to each other to achieve
...
NOTE: The author, at least, frequently forgets the <signal ... /> entry in the
...
and can therefore also be used in immediate execution mode without a
defined function that contains the loop.

==== Waiting for specific Event

In more complicated cases, it might be needed to decode which widget has sent
which event:
