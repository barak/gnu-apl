...
GNU APL sources (i.e. after an SVN checkout or after unpacking the GNU APL tar
...
*5 FILE_IO* and *⎕FIO[201]* respectively.

CAUTION: Parallel execution is an entirely experimental feature that should
not be used in mission-critical applications. Support for bugs caused by
this feature is rather limited.

== Controlling the Core Count

The number of cores that are used in parallel APL execution can be
controlled in several ways.

=== The ./configure Option CORE_COUNT_WANTED

The first way to control the (maximum) core count is at the point in time when
the interpreter source code is being ./configure'd. The configure option
...
*CORE_COUNT_WANTED=-3* is the most useful setting, because it allows changing
...
particular *⎕SYL[24 25 26;]* control the number of cores being used:
...
indicated by the platform. The value 12 above was reported on an i7-8700
...
* *Parallel::init()* initializes semaphores related to parallel execution
  ...
  determined by *CORE_COUNT_WANTED* or by the *-cc* command line option
  (no checks are performed to verify that the CPUs chosen are correct).
  Otherwise, i.e. (*CORE_COUNT_WANTED* is -1 or -3) the cores available to
  the interpreter are determined by *pthread_getaffinity_np()* and all CPUs
  that are available to the interpreter are stored in the vector.
...
a sufficiently large argument, then it unleashes the workers
...
After initialization, the parallel subsystem works (see *Thread_context.cc/hh*)
as follows.
...
* every thread maintains a variable *job_number* which is initially 0 for both
...
commonly known as the *speed-up* of the parallel execution. The difference
⍺~par~ - ⍺~seq~ is primarily caused by functions like M_fork(), PF_fork(),
...
T~seq~(N) and T~par~(N) is the start-up time ⍺~seq~ and ⍺~par~ respectively.
...
made it practically impossible to predict the execution time of any given code.
...
This one went less well. One difference from the previous one is that the
...
larger than the parallel start-up time ⍺~par~.
...
* every measurement needs to be visualized (plotted) to rule out too many
  or too large outliers.
...
* The speed-up of additional virtual cores (compared to physical ones) seems
  to be rather low. That is, for example, the speed-up of 12 virtual cores (on
...
precise than old-fashioned measurements using ⎕TS at the APL level.
...
Therefore cores #1 and #4 compute one sum, core #2 computes 3 sums, and
core #3 computes 1000 sums. This is obviously not optimal since cores #1, #2,
...
One purpose of benchmarking is to find the break-even lengths for
scalar functions. After that length is found, one can inform the APL
...
rank) with fewer than 33 ravel items will be computed sequentially, but longer
arrays in parallel.
...
The last plot window shown by the *Scalar2* workspace summarizes the speed-ups
...
The speed-ups plotted in the last window are usually reasonable if:
...
The measurement shown above was executed on a 6-core i7-8700 CPU. The CPU has
...
* *-f 39* : selects *Z←¯6○B* aka. *arccosh(B)* as the benchmarking function
  with a mix of Integer, Real, and Complex numbers,
...
measurement with 12 cores would have shown a rather bad speed-up (try it
