...
code of that quantity is not as trivial as finding faults in a short program
of only a few thousand lines. Past experience shows, that after a new fault
was observed, the time for reproducing and localizing the fault typically
takes more than 90% of the total time to fix it.
...
Errors in user programs are reported by the APL interpreter when APL code
is being executed. Before the interpreter starts its internal computation(s)
...
An APL error is displayed immediately and (provided that output coloring
...
* An error was raised incorrectly, i.e. for valid APL input, or
...
If *GNU APL* produces an incorrect result, then you should report it in an
...
to be *true* (aka. ≠ 0 in C/Cpp). It can be very hard to find the origin
(root cause) of an error, in particular when many CPU instructions were
performed between the root cause and error being detected.

By making assertions all over place in the source code, the programmer
introduces checks along the execution path in order to reduce the number of
CPU instructions between a root cause and the point in time where it is
being detected. In the C/Cpp code, assertions are typically made in the
following places:
...
Assertions are implemented as one or the other of two C/Cpp macros called
Assert() and Assert1(). As of this writing there are more than 530 Assert()
and more than 110 Assert1() macros spread across the *GNU APL* source code.
The reason for having different Assert macros is their impact on performance.
...
large number of Assert() macros increases the total performance impact which
...
* *ASSERT_LEVEL_WANTED = 1* (the default): this disables the Assert1() macro
...
which, besides other settings, does *./configure* with ASSERT_LEVEL_WANTED=2,
thus enabling both Assert macros and possibly making it easier to find the
root cause of a fault.

As confidence with the source code increases over time, Assert() macros will
eventually be converted to Assert1() macros while new code will initially
...
The C/Cpp stack above tells us which assertion has failed and where the
assertion is located in the C/Cpp source code (i.e. file Quad_FIO.cc line 1085),
while the )SI stack tells us where that location is in the APL code.

This brings us closer to the root cause of a problem. Unfortunately the hex
addresses numbers on the left side of the C/Cpp stack dump are process
specific, i.e. the same *⎕FIO ¯16* will produce different numbers in a
different process of the operating system.

=== Line numbers in Stack Traces

If the functions on the right side of the C/Cpp stack are relatively small (as
most functions in the source code of *GNU APL* are) then we can usually find the
exact source line for each stack entry rather easily using the function name
...
hex addresses on the left into source line numbers as explained in the
following.

First of all the Cpp compiler must be instructed to not discard line numbers.
...
directory, the make target *apl.lines* creates this file (which takes quite a
...
. The source line numbers are now displayed for most hex numbers. Some
  addresses can not be resolved, this could be caused by C/Cpp function
...
any longer via either the symbol table or the interpreter (like an APL
variable) or the body of a defined function (like an APL literal).

If a )CHECK command fails, then it is important to remember, or better write
down, which sequence of APL operations has led to the state of the workspace.
...
*GNU APL* provides a different debugging tool called *Value history*.
...
and therefore requires recompilation of the interpreter.
...
the crashing binary is started. In GNU/Linux with its *bash* shell that means:
...
in directory src below the top--level-directory) you can start the debugger
*gdb* in the src directory with the core file as its second argument:
...
stack dump similar to the one discussed earlier. The *GNU APL* maintainer(s)
will appreciate a copy of that stack dump for problems that are difficult to
reproduce.
...
yours) and not on others.

The first thing to locate their root cause is to *./configure* with either no
options or via *make develop* to see if the error is related to specific
*./configure* options.

Also, look for compiler warnings when *GNU APL* is built. In the default build,
compiler warnings can be overlooked easily. *make develop* turns most warnings
into errors so you will see at the end if the build was successful.

=== Optimization

Recently we have seen faults that may have been caused by an overly aggressive
optimization of the *GNU APL* Cpp source code. These faults were observed for
the same source code but only when compiled with newer versions of the
same g++ compiler.

By default *GNU APL* compiles with optimization level -O2. If a fault is
suspected to be caused by optimization, then it could make sense to also
test the same source code of *GNU APL* without any optimization.
...
The large (and increasing) number of logging facilities cause the tests,
whether Log() macro or its respective logging facility is ON or OFF, to have
significant performance impacts. For this reason, *GNU APL* provides two
...
*./configure* time), see also *README-2-configure*.
...
With Static logging, the selection of which logging facility shall be ON is
...
interpreter was started, the value of each variable can be changed from
OFF to ON (and vice versa) with the *GNU APL* debug command *]LOG* (which
toggles the state of the respective variable).

* Disadvantage: Dynamic Logging has a small performance impact (even
  if all variables are set to OFF) which will accumulate for all Log() macros,
...
because changing the logging facilities at run-time requires dynamic logging.

----
      ]LOG

Command ]LOG is not available, since dynamic logging was not configured
for this APL interpreter. To enable dynamic logging (which will slightly
decrease performance), recompile the interpreter as follows:
...
Many of the currently over 40 logging facilities serve a particular purpose
for the maintainers or have become obsolete and are therefore not too useful
for the trouble-shooting performed by the user. The others are mentioned below.
...
in the APL code, but none as to where that error was detected on the C/Cpp
code. These logging facilities add this information.

==== ]Log 32 Prefix Parser

Parsing of APL commands and Expressions. To analyze SYNTAX ERRORs.
...
To simplify the source code for printouts, *GNU APL* has defined two powerful
macros *LOC* and *Q()*. The *Q()* macro is used exclusively for intrusive
troubleshooting while the LOC macro is also used for other purposes in the
source code (with more than 1700 invocations as of this writing).
...
How does this work? Suppose we have a 13-line Cpp file *test.cc* like this:
...
This is quite some work for the compiler, but the result is such that the
run-time cost for *GNU APL* is only the loading of the string literal when
*GNU APL* starts.
...
In this definition *get_CERR()* is a function which returns an output channel
...
with many different data types without the burden to properly format them
...
A frequent question is not the value of a Cpp variable, but rather if (and
...
insertions of *Q()* macros in various source files (and hopefully having
...
have compiled and installed the latest GNU APL version from subversion (SVN)
or git.
...
* how the defined functions in the workspace appear, how they interact,
