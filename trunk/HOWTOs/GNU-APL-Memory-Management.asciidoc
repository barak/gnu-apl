...
the consequences are for a GNU APL programmer.
...
|===============
| Binary             | Size (bytes)
|GNU APL Interpreter |  44,620,848
|APserver            |   1,540,944
|Gtk_server          |     530,376
|===============
...
* After the interpreter has started it requests additional memory from the
  ...
  memory is allocated by malloc(), either directly or indirectly (via Cpp
  new()). When the memory is no longer needed, it is returned to malloc()
  memory management (for later re-use). The memory kept in the malloc()
  memory management is only returned to the operating system when the
...
sizes, there is a (very) small overhead in the malloc() library for every
object. Each of the objects is significantly larger than its malloc overhead
and therefore the overhead can normally be ignored. A typical APL workspace is
assumed to have fewer than 1000 APL symbols (i.e. variables or defined
functions) and an )SI stack depth of less than 100. A typical computer on which
GNU APL runs is assumed to have 4 GB of memory available to the interpreter and
it should definitely have at least 1 GB.
...
for the different objects is explained below.

=== X for a Unicode string (aka. UCS string)

X is 4 bytes per Unicode character. For performance reasons at least 16
characters are allocated so that X ≥ 64.

=== X for an APL Symbol

An APL symbol has two variable parts: a UCS string containing the name of the
symbol (e.g. a variable name or the name of a defined function) and a value
...
When a defined function is created, its textual representation (i.e. the
argument for ⎕FX or the result produced by the ∇-editor) is stored in the
function object for the purpose of displaying error information in case the
execution of the function fails.

The textual representation is then converted into an internal representation
of fixed-sized Tokens with one token for each lexical unit. A lexical unit is:
...
* Utx is the size of the UCS_string containing the function text,
...
efficient than the allocation or deallocation of larger areas. Finally, short
APL values occur more frequently in APL programs than long values, and they
...
value had the same type such as bit, 8-bit char, 32-bit integer, or float and
the ravel was densely packed. Accessing a particular element V[n] of a value V
was a fast constant time operation, which was internally performed very
...
briefly considered, but then ruled out as well because code size for the second
option was expected to be significantly larger than for the third option.
...
This value was determined at the start-up of the interpreter (where the malloc()
...
* A direct consequence of virtual memory is that the size of it is no longer
  ...
  longer a constant but becomes dependent on what happens not only in the APL
...
* some other (unrelated) process may be killed by a memory request from GNU
  APL, or
* GNU APL is killed as a result of its own memory request.
...
system limit) entries that can be set by an APL program to prevent GNU APL
from requesting too much memory. Some of them have to be combined with others
...
* Finally, ⎕SYL[31 32;] define a safety margin that is explained below.
...
values, and ⎕SYL[3;] protects against APL values becoming too large.
...
The actual implementation of *MemAvailable:* by the kernel is quite complicated
(more than simply adding *MemFree:* and *Cached:*). For that reason, even if
*MemAvailable:* were available on all GNU/Linux machines it would still be
impossible to call it before each creation of an APL value.
...
** If the process that runs the interpreter has a limit set on the amount of
   its virtual memory, then the interpreter assumes that amount of memory
...
apl
----
The unit is kB, so that the command above will set the limit to 1 GB. The limit
remains in force for all subsequent commands and can be removed like this:
...
problem that a process may have when it reaches the limit. However, it helps a
process to predict when the limit will be reached.
...
WS FULL can occur very many places within the interpreter. To find the exact
...
computation of *N ⍴ A* creates exactly 1+N values: a large value for the result
...
1. if the first (large) allocation fails, then the free memory is still F,
...
Exception handling with only 1024 bytes left can fail as can be demonstrated
on a 32-bit machine quite easily. The following examples show both cases (with
...
* The stack dumps differ. The pretty-printing of Cpp function names requires
...
* The )MORE information in the two examples differ. The term *Ravel
  allocation* in the printout of example 1 refers to the allocation of the
  (large) top-level ravel.
...
* It helps a lot if you can provide a reliable way to reproduce the problem.
...
* Check that the problem cannot be fixed by setting memory limits. As explained
  above there are some boundary conditions from the operating system that
...
** ⎕SYL[1 2 3;2] to limit what the GNU APL interpreter may try to allocate, and
