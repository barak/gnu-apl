...
A *server* is a program or process that is controlled by one or more other
programs or processes. A *client* is a program or process that controls
...
communication. In client/server communication, it is the client that
...
Some partners are started beforehand by the operating system (Web servers,
SQL servers), some are started automatically by GNU APL (*APserver()* for
shared variables) and some may need to be started by other means before
...
(causing a SYNTAX ERROR) or raise a DOMAIN ERROR. The fact that GNU APL is
in one role, for example as server, does not preclude it from being a client
in a different communication context.
...
== GNU APL Communication Mechanism Details
...
mode). *REPL* stands for **R**ead-**E**valuate-**P**rint-**L**oop and that
describes the top-level operation of an APL interpreter in interactive mode.

If GNU APL is *./configure'd* with option *--with-libapl*, then GNU APL does
not build a normal executable that can be started from a script or from a
shell, but instead it builds a library *libapl*. That library can then be
linked with another program that either implements its own *REPL* loop
(thus providing some interactive mode, but with a different user interface),
or only calls GNU APL facilities from time to time.
...
And running the newly created *test_libapl* produces what would be expected:
...
that a user can add more APs. It seems, however, that most of the APs that
were not implemented have been superseded by more modern interfaces (most
of which are sub-functions of ⎕FIO) that are easier to use than shared
variables.

For workspace-to-workspace shared variables, the APserver stores the shared
variables themselves, so that no data is lost if one of the connected
workspaces exits or *⎕SVR*s a shared variable. APserver also maps the
...
More details on *⎕SQL* can be found in *info apl* (around Section 2.36).
...
on the APL side of the communication to send data to or receive data from the
spawned program using, for example, *⎕FIO[7] Handle* (aka *fwrite*) or
*⎕FIO[6] Handle* (aka. *fread*) respectively. The handle is normally closed
with *⎕FIO[4] Handle* (aka. *fclose*).
...
returns 0) in the spawned program. Under normal circumstances, the
communication should be closed from the client (= APL) side.
...
----
      Path ← '/usr/lib/apl/TLV_server'   ⍝ or wherever TLV_server was installed
...
"TLV buffers encoded with 33 ⎕CR sent to this program with ⎕FIO[43]\n"
        , strerror(errno));
        return 1;
      }
   ...
   for (;;)
       {
          ...
          if (rx_len != 8)
             {
               fprintf(stderr, "TLV socket closed (1): %s\n",
                       strerror(errno));
               fclose(f);
               return 0;
             }
          ...
          if (TLV_len)
             {
               ...
               if (rx_len != TLV_len)
                  {
                    fprintf(stderr, "TLV socket closed (2): %s\n",
                            strerror(errno));
                    fclose(f);
                    return 0;
                  }
          ...
          if (tx_len != (8 + TLV_len))
             {
               fprintf(stderr, "TLV socket closed (3): %s\n", strerror(errno));
               fclose(f);
               return 0;
             }
       }
...
these functions to set up sockets in a way that specific low-level properties
...
*info apl* (around section 2.26) provides some more details about ⎕FIO. One
can invoke ⎕FIO "" to obtain a list of ⎕FIO sub-functions, in particular the
names of the corresponding functions in the libstdc library.

The function names can then be used with the *man* command which documents the
...
particular those taking pointer arguments) are mapped to APL byte arrays.

The test case file *src/testcases/Quad_FIO.tc* contains APL code for a short
...
can get complete control by means of GNU APL native functions. A native
...
That makes the 3 signatures implemented in *FOO.so* available in APL (for the
same signatures).

*Note:* Native functions were not primarily intended for communication purposes.
But since they import the full power of C/Cpp into GNU APL they are a last
resort if all methods above cannot be used.

The directory *src/native* contains Cpp code templates for different APL
function signatures. You can use these templates as a starting point for your
own native function(s). If you modify a template in-place (i.e. you do NOT copy
and rename it) then the GNU APL build system will compile your changes and
create .so files in directory *src/native/.libs* that can be **⎕FX**ed later on.
...
while its inverse function *¯33 ⎕CR* (or *34 ⎕CR*) converts a TLV back to
